...
// PARALLEL small-model call judges whether it is WORK ("接活" — hands-on build/fix/change/handle OR a concrete
// deliverable: analyze-and-report, research-and-write-up, draft/design/translate) vs plain chat/Q&A, names the
// work item, and detects a "continue" follow-up. Same discipline as title.service:
...
Decide three fields:
1. "isWork" — true when the message hands the expert a JOB to do or deliver. That covers hands-on work (build, fix, change, create, configure, deploy, clean up, handle files/data/systems) AND producing a concrete deliverable: analyze data/logs/metrics and report the findings, research a topic and write it up, draft a report/plan/document, design a scheme or campaign, translate or proofread something, compile/organize material. Needing to read files or use tools to fulfil it still counts. NOT work — anything where a direct reply IS the whole answer: questions seeking an explanation or opinion ("why is X slow?", "which is better?"), quick factual lookups, casual "what does this say" reading, and chitchat.
...
// HOW, it doesn't hand over the fix). Deliverable-producing verbs (分析/复盘/翻译/analyze/draft…) count as
// work too — the boundary is "hands over a job or a deliverable" vs "a reply IS the answer", NOT "does it
// mutate something" (Studio's analyst/e-commerce positioning: "分析直播数据并输出报告" is an assignment).
...
const WORK_RE =
  /(修复|修一下|修个|修掉|修好|排查|解决|实现|添加|新增|加个|加一个|加上|改成|改掉|改一下|重构|优化|部署|上线|升级|迁移|集成|接入|删除|删掉|去掉|写个|写一个|写好|做个|做一个|生成一个|搭个|搭建|处理一下|处理这|清理|补上|补齐|帮我(修|改|加|写|做|删|查|建|部署|处理|清理|分析|整理|翻译|评估|调研)|分析(一下|下|一波)|分析[^。,,??]{0,12}(数据|日志|报表|销量|趋势|直播|原因|瓶颈|性能|情况)|做[^。,,??]{0,10}(分析|报告|复盘|方案|规划)|出[个一]?份?(报告|方案|计划|清单)|写[^。,,??]{0,6}(报告|方案|总结|文档|文案|稿子?|周报|日报)|设计[^。,,??]{0,10}(方案|页面|界面|海报|流程|活动)|(复盘|汇总|梳理|盘点|整理|调研|评估)(一下|下|这|一波)|翻译)|\b(fix|implement|add|append|build|create|refactor|deploy|migrate|integrate|remove|delete|rename|update|upgrade|write|make|set\s?up|debug|resolve|handle|patch|install|configure|optimize|clean\s?up|analy[sz]e|audit|translate|proofread|draft|compile|research|investigate|evaluate|assess|summari[sz]e)\b/i
