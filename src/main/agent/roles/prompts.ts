...
- One expert fits → {"mode":"single","role":"<name>","intro":"<one sentence to the user>","reason":"<≤8 words>","needsPlan":<boolean>,"investigate":<boolean>,"isWork":<boolean>,"taskTitle":"<job name>"}
- Sequential steps (one expert's output feeds the next) → {"mode":"pipeline","roles":["<name>",...],"intro":"<one sentence>","reason":"<≤8 words>","needsPlan":<boolean>,"investigate":<boolean>,"isWork":<boolean>,"taskTitle":"<job name>","roleTitles":{"<name>":"<their slice>"}}
...
- A project 2-3 builder experts BUILD TOGETHER, coordinating live as they go (e.g. a frontend that needs the backend's API — they work in parallel and message each other to integrate) → {"mode":"collaborate","roles":["<name>",...],"intro":"<one sentence>","reason":"<≤8 words>","needsPlan":<boolean>,"investigate":<boolean>,"isWork":<boolean>,"taskTitle":"<job name>","roleTitles":{"<name>":"<their slice>"}}
...
- "isWork": true when the user hands over a JOB — hands-on work (building / fixing / changing / creating / configuring / handling something real) OR a concrete deliverable (analyze data and report the findings, research and write something up, draft / design / translate / compile something; code or not; broader than needsPlan, which is code-change only). False for pure conversation where the reply IS the answer: questions, explanations, opinions, chitchat. With isWork true, include "taskTitle" (a concise 3-10 word job name, in the user's language) and — for multi-expert modes — "roleTitles" (each expert's own slice, keyed by their name).
