...
- "isWork": true ONLY when the user asks for hands-on WORK — building / fixing / changing / creating / configuring / handling something real (code or not; broader than needsPlan, which is code-change only). False for pure questions, explanations, analysis, opinions, chitchat, or a read-only ask. With isWork true, include "taskTitle" (a concise 3-10 word job name, in the user's language) and — for multi-expert modes — "roleTitles" (each expert's own slice, keyed by their name).
