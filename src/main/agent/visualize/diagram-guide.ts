// @generated from Claude Desktop 1.17377.2 (app.asar .vite/build/index.js), CC visualize/"Imagine" asset `nuo (width-parameterized: 680 desktop / 380 mobile)`.
// Byte-identical to CC — do not edit. Spec: docs/visualize-alignment-design.md §3; archive: docs/visualize-assets/diagram-guide.fn.js

export const diagramGuide = (A: number): string => {const e=`**Tier packing:** Compute total width BEFORE placing. Example — 4 pub/sub consumer boxes:
- WRONG: x=40,160,260,360 w=160 → 40-60px overlaps (4×160=640 > 480 available)
- RIGHT: x=50,200,350,500 w=130 gap=20 → fits (4×130 + 3×20 = 580 ≤ 590 safe width; right edge at 630 ≤ 640)`,t=`**Structural container example** (library branch with two side-by-side regions, an internal labeled arrow, and an external input). ViewBox 700x320, horizontal layout, color classes handle both light and dark mode — no \`<style>\` block:
\`\`\`svg
<defs>
...
<\/script>
\`\`\``:`**Illustrative diagrams at ${A}px**: the full interactive worked example (water-heater cross-section with thermostat slider, animated convection, and right-margin callouts) is omitted here — it was authored at 680px and would need a complete re-layout, not a coordinate scale, to fit. Follow the composition rules above with \`viewBox="0 0 ${A} H"\` directly: lay the subject tall-and-narrow at e.g. x=40..220, reserve x=230..${A-20} for right-side leader-line labels (\`class="ts"\`, \`text-anchor="start"\`), and stack controls below the SVG instead of beside it.`,i=A>400?`**Illustrative example — abstract subject** (attention in a transformer). Same rules, no physical object. A row of tokens at the bottom, one query token highlighted, weight-scaled lines fanning to every other token. Caption sits below the fan — clear of every stroke — not inside it.
\`\`\`svg
...
\`\`\``:`For abstract subjects (e.g. transformer attention) at ${A}px: same approach. The five-token fan-out example is omitted — at this width, lay out three tokens (x≈60, 160, 260; w≈80 each) with the middle one as the query, fan three weight lines, and put the caption below at y past the lowest stroke.`;return`## Diagram types
...
**Layout**: Prefer single-direction flows (all top-down or all left-right). Keep diagrams simple — max 4-5 nodes per diagram. The widget is narrow (~${A}px) so complex layouts break.
...
- Max 2-3 nesting levels. Deeper nesting gets unreadable at ${A}px width.
...
- **Layout follows the subject's geometry**, not a grid. If the thing is tall and narrow (a water heater, a thermometer), the diagram is tall and narrow. If it's wide and flat (a PCB, a geological cross-section), the diagram is wide. Let the subject dictate proportions within the ${A}px viewBox width.
...
All core rules still apply (viewBox ${A}px, dark mode mandatory, 14/12px text, pre-built classes, arrow marker, clickable nodes).
